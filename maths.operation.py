"""
We'll define a DAG where the tasks are follows:

Task 1: Start with the initialnumber (eg, 10).
...
def multiply_by_two(**context):
    current_value = context['ti'].xcom_pull(key = 'current_value', tasks_ids = 'start_task')
...
def subtract_three(**context):
    current_value = context['ti'].xcom_pull(key = 'current_value', task_ids = 'substract_by_three')
...
def square_number(**context):
    current_value = context['ti'].xcom_pull(key = 'current_value', task_ids = 'square by two')
    new_value = current_value ** 2
    context['ti'].xcom_push(key = 'current_value', value = new_value)
